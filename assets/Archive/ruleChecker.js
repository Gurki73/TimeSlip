...
function adjustRepeatLogic(repeatKey, timeframeDays) {
    // W2 = "entweder" (OR)
    if (repeatKey === "W2" && (!timeframeDays || timeframeDays.length <= 1)) {
        // fallback to mandatory AND phrasing
        return "W1";
    }
    return repeatKey;
}
...
function GetTimeBlock(timeframeDays, repeatKey, useAbbreviation = false) {
    if (!timeframeDays || timeframeDays.length === 0) return "Es sollten";

    const days = timeframeDays.map(d => useAbbreviation ? d.abbr || d.text : d.text);

    switch (repeatKey) {
        case "W1": // mandatory AND
            return `Jeden ${joinGermanList(days, "und")}`;
        case "W2": // OR
            return `Entweder ${joinGermanList(days, "oder")}`;
        case "W3": // ONLY / restriction
            return `Nur ${joinGermanList(days, "oder")}`;
        case "W4": // counts handled separately
            return joinGermanList(days, "und"); // count sentence will prepend "2 mal …"
        default:
            return "Es sollten";
    }
